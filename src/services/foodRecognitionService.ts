...
async function getTensorflowModule() {
  ...
  try {
    // Try direct require first (which works better with Metro bundler)
    ...
    if (!loadTensorflowModel) {
      ...
      console.error('TensorFlow Lite module:', tflite);
    ...
    console.error('Failed to load TensorFlow Lite module:', error.message);
    console.log('TensorFlow Lite not available - camera recognition will not work');
    return null;
  }
}
  ...
  async loadModel(): Promise<boolean> {
    ...
    try {
      ...
      if (!this.tfliteModule) {
        ...
        if (!this.tfliteModule) {
          console.warn('TensorFlow Lite module not available - this is expected in development mode');
          console.warn('Food recognition requires native device deployment');
          throw new Error('Camera recognition is not available in development mode. Please build and run on a physical device.');
        }
      }
