...
import { getTodayString, formatDateForDisplay } from '../../src/utils/dateUtils';
...
export default function ExerciseScreen() {
  ...
  const toggleExerciseUnit = (exerciseIndex: number) => {
    ...
    const updatedExercises = [...currentWorkout.exercises];
    const currentUnit = updatedExercises[exerciseIndex].unit || 'reps';
    updatedExercises[exerciseIndex].unit = currentUnit === 'reps' ? 'seconds' : 'reps';
          ...
          <View key={exercise.id} style={styles.exerciseSection}>
            <View style={styles.exerciseHeader}>
              {editingExerciseName === exerciseIndex ? (
                <View style={styles.exerciseNameEditContainer}>
                  ...
                  <View style={styles.exerciseActions}>
                    <TouchableOpacity
                      style={[styles.unitToggleButton, (exercise.unit || 'reps') === 'seconds' && styles.unitToggleButtonActive]}
                      onPress={() => toggleExerciseUnit(exerciseIndex)}
                    >
                      <Text style={[styles.unitToggleButtonText, (exercise.unit || 'reps') === 'seconds' && styles.unitToggleButtonTextActive]}>
                        {(exercise.unit || 'reps') === 'reps' ? '🔢' : '⏱️'} {(exercise.unit || 'reps') === 'reps' ? 'Reps' : 'Seconds'}
                      </Text>
                    </TouchableOpacity>
                    <TouchableOpacity
                ...
                <View style={styles.exerciseNameContainer}>
                  <TouchableOpacity onPress={() => startEditingExerciseName(exerciseIndex)} style={styles.exerciseNameTouchable}>
              ...
              <View key={set.id}>
                <View style={styles.setRow}>
                  ...
                  <DecimalInput
                    ...
                    value={set.weight > 0 ? parseFloat(getDisplayWeight(set.weight, 'imperial')) : undefined}
                  ...
                  <DecimalInput
                    style={styles.setInput}
                    placeholder={(exercise.unit || 'reps') === 'reps' ? 'Reps' : 'Seconds'}
                    value={set.reps > 0 ? set.reps : undefined}
                    onChangeValue={(value) => updateSet(exerciseIndex, setIndex, 'reps', value)}
                    allowDecimals={(exercise.unit || 'reps') === 'seconds'}
                    returnKeyType="done"
                  />
                  <Text style={styles.setLabel}>{(exercise.unit || 'reps') === 'reps' ? 'reps' : 'sec'}</Text>
                  <DecimalInput
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.header}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => setShowCalendar(true)}
          >
            <Text style={styles.date}>{formatDateForDisplay(selectedDate)}</Text>
          </TouchableOpacity>
        </View>
          ...
          {workoutSessions.slice(-10).reverse().map((session) => (
            <View key={session.id} style={styles.historyItem}>
              <TouchableOpacity 
                ...
                activeOpacity={0.7}
              >
                <Text style={styles.historyDate}>{formatDateForDisplay(session.date, { month: 'short', day: 'numeric', year: 'numeric' })}</Text>
            ...
            <View style={styles.workoutDetailsModalContent}>
              ...
              <View style={styles.workoutDetailsInfo}>
                <Text style={styles.workoutDetailsDate}>
                  📅 {viewingWorkout ? formatDateForDisplay(viewingWorkout.date) : ''}
                </Text>
                ...
                {viewingWorkout?.exercises.map((exercise, exerciseIndex) => (
                  <View key={exercise.id} style={styles.workoutDetailsExerciseSection}>
                    ...
                    <View style={styles.workoutDetailsSetsHeader}>
                      ...
                      <Text style={styles.workoutDetailsSetsHeaderText}>{(exercise.unit || 'reps') === 'reps' ? 'Reps' : 'Seconds'}</Text>
                    ...
                    {exercise.sets.map((set, setIndex) => (
                      <View key={set.id}>
                        <View style={styles.workoutDetailsSetRow}>
                          ...
                          <Text style={styles.workoutDetailsSetValue}>
                            {set.reps > 0 ? `${set.reps}${(exercise.unit || 'reps') === 'seconds' ? 's' : ''}` : '-'}
                          </Text>
                          <Text style={styles.workoutDetailsSetValue}>
                            {set.rpe ? set.rpe : '-'}
                          </Text>
